                                     ...
                                     struct wl_resource *resource)
{
    ...
    if (manager->ownerClients.value(uid, nullptr) == resource) {
        manager->ownerClients.remove(uid);
    }

    wl_resource_destroy(resource);
}

static void shortcut_manager_handle_aquire(struct wl_client *client,
                                           struct wl_resource *resource)
{
    auto *manager = shortcut_manager_from_resource(resource);
    ...
    uid_t uid;
    wl_client_get_credentials(client, nullptr, &uid, nullptr);
    if (manager->ownerClients.contains(uid)) {
    ...
    manager->ownerClients.insert(uid, resource);
}
                                             ...
                                             const uint action)
{
    ...
    Q_EMIT manager->requestBindKeySequence(uid,
                                                       ...
                                                       const uint action)
{
    ...
    Q_EMIT manager->requestBindSwipeGesture(uid,
                                            QString::fromUtf8(name),
                                            finger,
                                            direction,
                                            action);
                                                      ...
                                                      const uint action)
{
    ...
    Q_EMIT manager->requestBindHoldGesture(uid,
                                           QString::fromUtf8(name),
                                           finger,
                                           action);
}
                                           ...
                                           struct wl_resource *resource)
{
    ...
    Q_EMIT manager->requestCommit(uid);
}
                                           ...
                                           const char *name)
{
    ...
    Q_EMIT manager->requestUnregisterShortcut(uid, QString::fromUtf8(name));
}

struct treeland_shortcut_manager_v2_interface shortcut_manager_impl {
    .destroy = shortcut_manager_destroy,
    .acquire = shortcut_manager_handle_aquire,
...
void treeland_shortcut_manager_v2::sendActivated(uid_t uid, const QString& name)
{
    wl_resource *resource = ownerClients.value(uid, nullptr);
    if (!resource) {
        return;
    }

    treeland_shortcut_manager_v2_send_activated(resource, name.toUtf8().constData());
}

void treeland_shortcut_manager_v2::sendCommitStatus(uid_t uid, bool success)
{
    wl_resource *resource = ownerClients.value(uid, nullptr);
    if (!resource) {
        return;
    ...
    treeland_shortcut_manager_v2_send_commit_status(resource, success);
}

void treeland_shortcut_manager_v2::sendInvalidCommit(uid_t uid)
{
    wl_resource *resource = ownerClients.value(uid, nullptr);
    if (!resource) {
        return;
