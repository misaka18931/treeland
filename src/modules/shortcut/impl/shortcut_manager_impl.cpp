                                             ...
                                             const char *keystr)
{
    ...
    uid_t uid = shortcut->uid;
    auto &userKeyMap = manager->keyMap[uid];
    if (userKeyMap.contains(keys)) {
        treeland_shortcut_v1_send_bind_failure(resource, TREELAND_SHORTCUT_V1_BIND_ERROR_CONFLICT);
        return;
    }

    uint binding_id = shortcut->newId();

    userKeyMap[keys] = shortcut;

    shortcut->bindings[binding_id] = std::unique_ptr<void, std::function<void(void*)>>(
        nullptr,
        [manager, uid, keys](void*) {
            auto &userKeyMap = manager->keyMap[uid];
            userKeyMap.remove(keys);
        });
    shortcut->sendBindSuccess(binding_id);
}
                                              ...
                                              uint32_t direction)
{
    ...
    if (d == SwipeGesture::Direction::Invalid) {
        treeland_shortcut_v1_send_bind_failure(resource, TREELAND_SHORTCUT_V1_BIND_ERROR_INVALID_DIRECTION);
        return;
    }

    auto gestureSignature = std::make_pair(d, finger);

    uid_t uid = shortcut->uid;
    auto &userBindedGestures = manager->bindedGestures[uid];
    if (userBindedGestures.contains(gestureSignature)) {
        treeland_shortcut_v1_send_bind_failure(resource, TREELAND_SHORTCUT_V1_BIND_ERROR_CONFLICT);
        return;
    }
    ...
    manager->bindedGestures[uid].insert(gestureSignature);
    TogglableGesture * gesture = new TogglableGesture(shortcut);
    gesture->addTouchpadSwipeGesture(d, finger);
    QObject::connect(gesture, &TogglableGesture::activated, shortcut, &treeland_shortcut_v1::gestureActivated);

    shortcut->bindings[binding_id] = std::unique_ptr<void, std::function<void(void*)>>(
        nullptr,
        [manager, uid, gestureSignature, gesture](void*) {
            auto &userBindedGestures = manager->bindedGestures[uid];
            userBindedGestures.remove(gestureSignature);
            gesture->disconnect();
            gesture->deleteLater();
        });
    shortcut->sendBindSuccess(binding_id);
                                              ...
                                              uint32_t finger)
{
    ...
    auto gestureSignature = std::make_pair(SwipeGesture::Direction::Invalid, finger);
    ...
    manager->bindedGestures[uid].insert(gestureSignature);
    TogglableGesture * gesture = new TogglableGesture(shortcut);
    gesture->addTouchpadHoldGesture(finger);
    QObject::connect(gesture, &TogglableGesture::longPressed, shortcut, &treeland_shortcut_v1::gestureActivated);
                                            ...
                                            uint32_t action)
{
    ...
    if (shortcut->actions.contains(*a)) {
        treeland_shortcut_v1_send_bind_failure(resource, TREELAND_SHORTCUT_V1_BIND_ERROR_CONFLICT);
        return;
    }
    ...
    shortcut->actions.append(*a);
    shortcut->bindings[binding_id] = std::unique_ptr<void, std::function<void(void*)>>(
        nullptr,
        [shortcut, a](void*) {
            shortcut->actions.removeAll(*a);
        });
    shortcut->sendBindSuccess(binding_id);
}
                                        ...
                                        uint32_t binding_id)
{
    ...
    shortcut->bindings.erase(binding_id);
}
...
uint treeland_shortcut_v1::newId()
{
