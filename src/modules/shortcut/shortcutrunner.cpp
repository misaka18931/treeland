...
void ShortcutRunner::onActionTrigger(ShortcutAction action, const QString &name)
{
    ...
    case ShortcutAction::Notify:
        helper->m_shortcutManager->sendActivated(name);
        break;
    ...
    case ShortcutAction::TaskSwitchQuickAdvance:
    {
        auto currentWs = helper->workspace()->current();
        if (currentWs) {
            auto next_surface = currentWs->findNextActivedSurface();
            if (next_surface) {
                helper->forceActivateSurface(next_surface, Qt::TabFocusReason);
            }
        }
        break;
    }
...
void ShortcutRunner::onActionFinish(ShortcutAction action, const QString &name)
{
    Q_UNUSED(name);
    switch (action) {
    case ShortcutAction::PrevWorkspace:
