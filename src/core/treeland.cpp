    ...
    if (CmdLine::ref().run().has_value()) {
        auto exec = [runCmd = CmdLine::ref().run().value(), d] {
            ...
            if (auto cmdline = CmdLine::ref().unescapeExecArgs(runCmd); cmdline) {
                ...
                envs.insert("WAYLAND_DISPLAY", d->sessionManager->globalWaylandSocket()->fullServerName());
                if (auto *xwayland = d->sessionManager->xwaylandForUid(getuid())) {
                    envs.insert("DISPLAY", xwayland->displayName());
                } else if (auto *current = d->sessionManager->globalXWayland()) {
        ...
        WSocket *defaultSocket = d->sessionManager->globalWaylandSocket();
...
bool Treeland::ActivateWayland(QDBusUnixFileDescriptor _fd)
{
    ...
    WSocket *sessionSocket = d->sessionManager->waylandSocketForUid(uid);
    if (!sessionSocket)
        return false;
...
void Treeland::XWaylandName()
{
    ...
    auto *xwayland = d->sessionManager->xwaylandForUid(uid);
    if (!xwayland) {
