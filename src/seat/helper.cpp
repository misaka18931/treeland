...
bool Helper::beforeDisposeEvent(WSeat *seat, QWindow *, QInputEvent *event)
{
    ...
    if (!m_captureSelector && m_currentMode == CurrentMode::Normal
        ...
        do {
            ...
            auto user = m_userModel->currentUser();
            if (m_shortcut->handleKeySequence(user ? user->UID() : getuid(), sequence)) {
                return true;
            }
...
void Helper::handleCompositorAction(treeland_shortcut_v1_action action) {
    switch (action) {
        ...
        case TREELAND_SHORTCUT_V1_ACTION_WORKSPACE_6:
            ...
            workspace()->switchTo(static_cast<int>(action)
                          - static_cast<int>(TREELAND_SHORTCUT_V1_ACTION_WORKSPACE_1));
            break;
...
#ifndef DISABLE_DDM
            if (m_lockScreen && m_lockScreen->available() && m_currentMode == CurrentMode::Normal) {
                showLockScreen();
            }
#endif
            break;
        ...
        case TREELAND_SHORTCUT_V1_ACTION_QUIT:
            Q_EMIT requestQuit();
            break;
        case TREELAND_SHORTCUT_V1_ACTION_CLOSE_WINDOW: // Original binding: Alt+F4
            if (m_activatedSurface) {
                m_activatedSurface->requestClose();
            }
            break;
        case TREELAND_SHORTCUT_V1_ACTION_SHOW_WINDOW_MENU: // Original binding: Alt+Space
            if (m_activatedSurface) {
                Q_EMIT m_activatedSurface->requestShowWindowMenu({0,0});
            }
            break;
        case TREELAND_SHORTCUT_V1_ACTION_TASKSWITCH_QUICK_ADVANCE: // Original binding: Rapid Alt then Tab (<150ms)
        {
            auto currentWs = workspace()->current();
            if (currentWs) {
                auto next_surface = currentWs->findNextActivedSurface();
                if (next_surface) {
                    forceActivateSurface(next_surface, Qt::TabFocusReason);
                }
            }
            break;
        }
        case TREELAND_SHORTCUT_V1_ACTION_TASKSWITCH_NEXT: // Original binding: Alt+Tab
        {
            if (!(m_currentMode == CurrentMode::Normal || m_currentMode == CurrentMode::WindowSwitch)) {
                break;
            }
        ...
        case TREELAND_SHORTCUT_V1_ACTION_TASKSWITCH_PREV: // Original binding: Alt+Shift+Tab
        {
            if (!(m_currentMode == CurrentMode::Normal || m_currentMode == CurrentMode::WindowSwitch)) {
                break;
            }
